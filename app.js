...
function setLanguage(){
  document.getElementById('score-hit').textContent = 'Unesite rezultat koji treba da se postigne';
  document.querySelector('.pt').classList.toggle('active');
  document.querySelector('.en').classList.toggle('active');
  document.getElementById('start-game').textContent = 'ZAPOČNITE';
  document.querySelector('.btn-new').innerHTML = '<i class="ion-ios-plus-outline"></i>NOVA IGRA';
  document.querySelector('.btn-roll').innerHTML = '<i class="ion-ios-loop"></i>BACI KOCKU';
  document.querySelector('.btn-hold').innerHTML = '<i class="ion-ios-download-outline"></i>POTVRDI';
  document.querySelector('.player-0-panel .player-current-label').textContent = 'trenutni';
  document.querySelector('.player-1-panel .player-current-label').textContent = 'trenutni';
}
...
document.querySelector('.btn-hold').addEventListener('click', function(){
  if(gamePlaying){
    ...
    if(scores[activeplayer] >= scoreHit){
      document.getElementById('name-' + activeplayer).textContent = (language == 'pt' ? 'Pobednik!' : 'Winner!');
      document.querySelector('.dice').style.display = 'none';
      document.querySelector('.player-' + activeplayer + '-panel').classList.remove('active');
